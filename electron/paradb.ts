 ...
 * Jitter spreads retries from many clients so they don't hammer the server
 * in lockstep after a rate-limit spike.
 */
 ...
 * third-party API use but asked us to be resilient to their rate limits:
 *   - 429 + 5xx → retry, honoring Retry-After when present, else backoff.
 *   - Network errors → retry with backoff.
 *   - Other 4xx → returned immediately (won't self-heal).
 */
